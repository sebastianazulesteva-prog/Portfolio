/* ═══ pdf-reader.js ═══
   ...
   THE READ TRAIL. One exception to that disposal: a page you have already read
   is kept, downscaled to TRAIL_PX, rather than blanked. Scrolling moves the
   strip upward, so everything read ends up overhead — look up mid-piece and
   you see the stack you have got through receding above you. See TRAIL_PX for
   the resolution/memory reasoning.
   ...
   THE READING RULER. The line you are looking at is highlighted, and clicking
   the page steps the highlight down a line — the flat site's Accessible-Mode
   line reader (`initA11yLineHighlight()` in /index.html), brought in here on
   Sebastian's ask. It lives in `reading-line.js`, which finds the lines in the
   page IMAGE (there is no text layer to ask, see above) and owns everything
   about them; this file hands it the page geometry and closes it again. The
   page planes are `.clickable` for it, which is the only reason a click on a
   page does anything at all.
...
(function () {
  ...
  // ── The read trail ──
  // Pages you have already read stay VISIBLE overhead instead of blanking out,
  // per Sebastian: scrolling moves the strip up, so everything read ends up
  // above you, and looking up should show the stack you have got through — the
  // spatial equivalent of a thumb-worth of pages in your left hand.
  //
  // They cannot all stay at reading resolution (that is the memory ceiling the
  // whole windowing scheme exists for), but they do not need to: a read page is
  // metres overhead and seen almost edge-on, so it only has to read as "a page
  // with text on it". When one leaves the reading window upward it is
  // re-rasterised from the image already in hand down to TRAIL_PX tall and the
  // full-size texture is dropped — no extra fetch, and ~200KB per page instead
  // of megabytes, so a 28-page piece costs single-digit MB for the whole trail.
  var TRAIL_PX = 256;
  // Safety valve for a pathologically long document; 28-page pieces never hit
  // it. Beyond this, the oldest pages blank as before.
  var TRAIL_MAX_PAGES = 60;
  ...
  var state = {
    ...
    guideBtn: null,     // the reading-guide toggle, on the rail's axis
    guideOn: true,      // on by default, per Sebastian; reset on every open
  ...
  // ── The reading ruler (reading-line.js) ──────────────────────────────────
  // Sebastian's ask: the flat site's line highlighter, in here. That file owns
  // all of it — finding the lines in the page image, the band, the gaze
  // tracking and the click-to-step-down. The reader's only job is to hand it
  // the geometry it cannot derive on its own and to close it again, so this
  // stays the one place that knows how a page maps onto the strip.
  //
  // Resolved through the scene each time rather than cached: the system exists
  // from load, but a harness may not have loaded the file at all, and a reader
  // that hard-required it would take the whole reading room down with it.
  ...
  // switched it off once to look at a diagram opens their next piece with the
  // ruler mysteriously gone.
  function setGuideEnabled(on) {
    state.guideOn = !!on;
    var rl = readingLine();
    if (rl) rl.setEnabled(state.guideOn);
    if (state.guideBtn) {
  ...
  function openReadingLine(root) {
    var rl = readingLine();
    if (!rl || !state.stripEl) return;
    rl.begin({
      strip: state.stripEl,
      pages: state.pages,
      pageW: state.pageW, pageH: state.pageH,
      step: state.step, topY1: state.topY1,
      readDistance: READ_DISTANCE,
      eyeHeight: EYE_HEIGHT,
      // Handed in as functions, not values: stepping the ruler past the bottom
      // of the comfortable reading band scrolls the strip, and it has to go
      // through the reader's OWN eased scroll so a stepped line and a tapped
      // rail move the page identically (and so the rail's thumb and the page
      // counter stay in agreement, which a direct write to state.scroll would
      // silently skip).
      scrollOf: function () { return state.scroll; },
      scrollBy: scrollBy
    });
    // begin() resets the system's own enabled flag to whatever it was; assert
    // the reader's default over the top of it, which is also what refreshes the
    // toggle's label for this piece.
  ...
  function loadPage(rec, onSettled) {
    ...
    VRGlass.loadTexture(rec.url, function (tex) {
      ...
      // A page that has since scrolled ABOVE the window is still wanted, just
      // at trail resolution — downscale from the image that just arrived
      // rather than discarding it and leaving a blank overhead.
      if (!state.open) {
        tex.dispose();
      ...
      if (!rec.wanted) {
        var demoted = trailTextureFrom(tex);
        if (demoted && !rec.trailTex) {
          rec.trailTex = demoted;
          rec.material.map = demoted;
      ...
      rec.material.needsUpdate = true;
      // Scrolling back up re-promotes a trail page: it keeps showing its
      // low-res self until the full image lands, so re-reading sharpens
      // instead of flashing through the placeholder tone.
      disposeTrail(rec);
  ...
  // Re-rasterise an already-loaded page image at trail resolution. Uses the
  // decoded HTMLImageElement hanging off the full texture, so this is pure
  // canvas work — no network, no pdf.js.
  function trailTextureFrom(fullTex) {
    var img = fullTex && fullTex.image;
    if (!img || !img.width || !img.height) return null;
    var h = Math.min(TRAIL_PX, img.height);
    var w = Math.max(1, Math.round(img.width * (h / img.height)));
    var c = document.createElement('canvas');
    c.width = w; c.height = h;
    try {
      c.getContext('2d').drawImage(img, 0, 0, w, h);
    } catch (e) {
      return null; // tainted/undecodable — fall back to blanking the page
    }
    var t = new THREE.CanvasTexture(c);
    // Same colour-space tagging the shared loader applies; without it these
    // come out visibly darker and warm-shifted than the pages still at full
    // resolution, which would make the seam at the top of the reading window
    // obvious (VR_AI_BUILD_GUIDE.md 3.5 is the same class of bug).
    t.colorSpace = THREE.SRGBColorSpace;
    // Read pages are viewed at a glancing angle from below, which is exactly
    // the case anisotropic filtering exists for.
    t.anisotropy = 8;
    return t;
  }

  // Leaving the reading window UPWARD: keep the page, drop it to trail
  // resolution. Falls back to a normal dispose if the downscale is not
  // possible (never loaded, or the draw failed).
  function demoteToTrail(rec) {
    if (rec.trailTex) {                 // already demoted
      if (rec.texture) { rec.texture.dispose(); rec.texture = null; }
      return;
    }
    if (!rec.texture) return;           // nothing loaded to downscale from
    var t = trailTextureFrom(rec.texture);
    if (!t) { disposePage(rec); return; }
    rec.trailTex = t;
    rec.material.map = t;
    rec.material.color.set('#ffffff');
    rec.material.needsUpdate = true;
  ...
  function disposeTrail(rec) {
    if (!rec.trailTex) return;
    rec.trailTex.dispose();
    rec.trailTex = null;
  }
  ...
  function updateRenderWindow(force) {
    ...
    state.pages.forEach(function (rec) {
      ...
      if (rec.wanted) { wanted.push(rec); return; }
      // ABOVE the reading window = already read. Keep it, at trail resolution,
      // so looking up shows the stack you have read rather than a column of
      // blank slabs. BELOW = not read yet, and there is nothing to show.
      var isRead = rec.index < current - RENDER_WINDOW;
      if (isRead && (current - rec.index) <= TRAIL_MAX_PAGES) demoteToTrail(rec);
  ...
  function build(project, entry) {
    ...
    for (var i = 0; i < pageCount; i++) {
      ...
      // The page is a POINTING SURFACE now, not just scenery: reading-line.js
      // reads the pointer's uv off it to place the reading ruler, and a click
      // on it steps the ruler down a line. `.clickable` is what puts it in the
      // three raycasters' object lists — nothing else here reacts to a click,
      // and the rail and the exit button both sit nearer the eye, so they keep
      // winning the hit test where they overlap.
      ...
      state.pages.push({ el: pageEl, mesh: mesh, material: mat, texture: null,
                         trailTex: null, loading: false, index: i, wanted: false,
                         url: pageUrl(state.entry, i),
                         // reading-line.js's cache of this page's detected text
                         // lines. `undefined` = not analysed yet, `null` = tried
                         // and there is nothing to find. Deliberately NOT reset
                         // by disposePage/demoteToTrail: the bands are numbers,
                         // so a page that scrolls out and back never pays for a
                         // second canvas read.
                         bands: undefined });
    }
  ...
  function buildScrollControl(root, project) {
    ...
    // ── The reading-guide toggle ──────────────────────────────────────────
    // Sebastian: *"add a button to turn it off by the controls, but have it be
    // turned on by default."*
    //
    // ON THE RAIL'S OWN AXIS, above the up pad. That column is already the
    // reader's control strip — up pad, thumb, down pad, page counter, all at
    // railX — so a fifth thing on it reads as part of the same object rather
    // than as a stray button floating beside the page. Above rather than below
    // because below is the page counter's, and because the up pad is the less
  ...
  function open(project) {
    ...
    firstPage(entry).then(function (firstTexture) {
      ...
      runTransition(function () {
        ...
        openReadingLine(root);
        // Also picks up the PAGE planes, which are .clickable now — without the
        // refresh the ruler has nothing to track until A-Frame's own
        // MutationObserver gets round to it, and that is the frame-timing gamble
        // this call exists to avoid (ISSUE-01).
  ...
  function close() {
    ...
    runTransition(function () {
      // Before the pages go: end() takes its listeners off each page entity and
      // disposes the band's geometry and program. removeChild does not free
      // either (trap §3.17), and this runs on every close, so skipping it is one
